...
file_path = path_to_skewers+"spec_xHeII1_007_ls_line{}.dat".format(skewer_number)
...
for i,L_i in enumerate(L_test_list): 
    ...
    skew_list.append(skewer_number)
    tsim_list.append(get_tsim(speed_test,delta_R_temp))
    #print("Skewer: {}\nLuminosity: {:.1e}\ntsim: {:.1f}\n".format(skewer_number,L_i,tsim))
    
stack = np.column_stack((np.asarray(skew_list,int),np.array(L_test_list),np.array(tsim_list)))
#print(stack)
