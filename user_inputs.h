...
namespace user_inputs
{
  ...
  inline constexpr bool spherical        { FALSE }; //If FALSE use plane-parallel
  ...
  inline constexpr char skewer[] {"0052"} ;
  inline constexpr char grid_input[] {"input_files/hydro_skewers/spec_xHeII1_007_ls_line0052.dat"};
  //inline constexpr double R_start         { 200.}; //kpc //choosing a starting point of skewer chunk
  inline constexpr double R_start         { 219.0}; //kpc //choosing a starting point of skewer chunk
  ...
  inline constexpr double scale_density_factor { 1 }; 

  //Uniform density skewers when spherical=False (plane-parallel), set to 0 for no time decay, this is artificial time decay
  //decay factor=(t/t0)^-eta
  inline constexpr double time_decay_index { 0 }; 
  inline constexpr double time_naught_decay { 20. }; //Myr 

  //output files
  inline constexpr char gas_output[] {"output_files/gas_hardRun_550myr.txt"};
  inline constexpr char initial_gas_output[]   { "output_files/gas_test_hydro_000myr.txt"};
  inline constexpr char otf_dir[] {"output_files/gasprops/vIF_test_sk0052"};
  //inline constexpr char otf_dir[] {"output_files/gasprops/csc_fd_sk0065_L1.45e+47_N_r300_v2"};
  ...
  inline constexpr int N_r { 6000 };
  inline constexpr int N_nu   { 30 }; //Number of frequency bins 50
  inline constexpr double t_max { 550 }; //Runtime (in Myr)
  inline constexpr double z        { 5.7 }; //redshift
  inline constexpr double R0       { 10000.0 }; //minimum radius of the grids, keep large to minimize geometric attenuation 
  inline constexpr double R        { R0+1500 };  //Total radius for the spatial grid (in pkpc) 
  ...
  inline constexpr double temp_0 { 5.e+1 }; //Initial gas temperature in K (if not input by user) 
  ...
  inline constexpr double Lum { 3.93e+45 }; 
  ...
  //inline constexpr double fH1_0      { 1.-0.9 };
  //inline constexpr double fHe1_0     { 1.-0.9 };
  //inline constexpr double fH2_0      { 0.9 };
  //inline constexpr double fHe2_0     { 0.9-1e-6 };
  inline constexpr double fH1_0      { 1.-1.e-6 }; //Initial HI fraction
  inline constexpr double fHe1_0     { 1.-1.e-6 }; //Initial HeI fraction
  inline constexpr double fH2_0      { 1.e-6 }; //Initial HII fraction
  inline constexpr double fHe2_0     { 1.e-6 }; //Initial HeII fraction  
  inline constexpr double fHe3_0     { 1.e-6 }; //Initial HeIII fraction
