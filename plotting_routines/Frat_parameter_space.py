...
#df = df[mask]
if uniform_density:
    df = pd.read_csv("/expanse/lustre/projects/uot171/bwils033/master_1d_rt/results/final_results/ud_otf.csv")
    df["locIF"]-=1e3
    mask = ((df["locIF"])>100)&(df["locIF"]<1667-100)&(df["F_inc"]>0)&(np.isfinite(df["F_lya"]))
    min_logvIF,max_logvIF = np.log10(3e2),np.log10(5e4)
...
bb = np.column_stack((v_IF,df["alpha_i"].values))
