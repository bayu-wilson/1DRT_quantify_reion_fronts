...
##skewer_number = str(sys.argv[1]) # 0001
#spectral_index = str(sys.argv[2]) # 0.500
...
skewer_number = "0004"
dir_path = "../output_files/gasprops/sk{}_a={}_Lum={}/".format(skewer_number,spectral_index,Lum)
fontsize=10

panel_matrix = [["I_nu",r"F$_{inc}$",r"$x_{HI}$(r)",r"$n_e \times n_p$ (r)"],
                [r"log nHI(r) [cm$^{-3}$].", r"F$_{Ly\alpha}$ (r)",r"log nHI(r) [cm$^{-3}$]",r"$q^eff_{Ly\alpha}$"],
                [None,r"F$_{Ratio}$ (r)","T(r) [K]",r"j$_{Ly\alpha}$(r)"]]
column_names = ["los pos[pkpc]","rho","ne","xHI_prev","n_tot","T[K]","xHI","xHeI","xHeII","gamma_HI","q_lya"]
fig_name_list = []
...
step_arr,t_arr, dt_arr,F_lya_otf,Finc_otf,locIF_otf,vIF_fd_otf,vIF_Flex_otf,TIF_otf, width_otf, nH_avg_otf, nH_center_otf, C_em_otf = table_otf.T#[mask]
...
while os.path.exists(gasprop_path):
    ...
    R0 = df_gasprops["los pos[pkpc]"].iloc[0]
    los_pos = df_gasprops["los pos[pkpc]"]-R0
    ...
    with open(gasprop_path,'r') as f:
        line = f.readline()
    step_n,t_n,dt_n,F_lya_n,Finc_n,locIF_n,vIF_fd_n,vIF_Flex_n,T_IF_n, width_n, nH_avg_n, nH_center_n, C_em_n = np.asarray(line.split(' \t'),float)
    vIF_fd_n = float(vIF_fd_n)/1e5
    vIF_Flex_n = float(vIF_Flex_n)/1e5
    ...
    ax[0][0].plot(nu_eV/13.6,I_nu*((locIF_calc+R0)*kpc_to_cm)**2)
    #ax[0][0].plot(nu_eV[n_minTime:n]/13.6,I_nu[n_minTime:n]*((locIF_calc+R0)*kpc_to_cm)**2)
    ax[1][0].plot(los_pos,np.log10(initial_nh1),color='gray')
    ax[1][0].plot(los_pos,np.log10(nHI))
    ax[0][1].scatter(np.log10(vIF_Flex_otf[n_minTime:n]/1e5),Finc_otf[n_minTime:n],s=1)
    ax[1][1].scatter(np.log10(vIF_Flex_otf[n_minTime:n]/1e5),F_lya_otf[n_minTime:n],s=1)
    ax[2][1].scatter(np.log10(vIF_Flex_otf[n_minTime:n]/1e5),(F_lya_otf/Finc_otf)[n_minTime:n],s=1)
    ax[0][2].plot(los_pos-locIF_calc,np.log10(df_gasprops["xHI"]))
    ...
    ax[0][4].plot(los_pos,nHI*df_gasprops["ne"])
    ax[1][4].plot(los_pos,df_gasprops["T[K]"])
    ax[2][4].plot(los_pos,j_lya)

    #ax[0][0].set_ylim(1e-25, 7e-24)
    ax[0][0].set_ylim(0.5e+25,5e+25)
    ...
    ax[2][2].text(-0.1,0.1,"n:                     {}\n".format(n)+
                      ...
                      "IF width (front):  {:.3f} pkpc \n".format(frontIF-locIF_calc)+
                      "IF width (back):   {:.3f} pkpc \n".format(locIF_calc-backIF)+
                      "IF width (total):  {:.3f} pkpc \n".format(frontIF-backIF)+
                      #"Em clumping factor: {:.3e}\n".format(C_em_n)+# +
                      #r"$n_H$(x$_{HI}=0.5$)"+":     {:.1e}".format(nH_value)+r" cm$^{-3}$"+'\n'+
                      "HI IF loc:            {:.1f} pkpc\n".format(locIF_calc)+
                      "HI IF vel (fd):      {:.1e}".format(vIF_fd_n)+r" km s$^{-1}$"+'\n'+
                      "HI IF vel (fm):     {:.1e}".format(vIF_Flex_n)+r" km s$^{-1}$"+'\n'+
                      r"I$_{Ly\alpha}$"+":                    {:.1e}".format(F_lya_n)+" s$^{-1}$ cm$^{-2}$ sr$^{-1}$"
                      ,transform=ax[2][0].transAxes)
    
    for j in range(rows):
    # ax[j][0].set_xlim(-10,310)
    # ax[j][1].set_xlim(0,320)
        ax[j][2].set_xlim(-35,50)
        ax[j][3].set_xlim(-35,50)
