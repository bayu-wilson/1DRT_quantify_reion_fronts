...
h = 6.626068e-27
pi = np.pi
columns = ['radius', 'rho', 'ne', 'ne_prev', 'dne_dt', 'n_tot', 'dt', 'T',
       'vel', 'nH1', 'fH1', 'fHeI', 'fHII', 'fHeII', 'fHeIII', 'j_lya']
columns_to_plot = ['T','rho','fH1','fHeI','dt','j_lya']

for i,sim_tot_time in enumerate(sim_tot_times):
    data_path=opt.output_dir+"gas_test_hydro_{}myr.txt".format(str(sim_tot_time))
    # radius, rho, ne, temp, vel, nh1, fh1, fhe1, fh2, fhe2, fhe3, q_eff_lya = rw.read_gas(data_path)
    # radius, rho, ne, temp, vel, nh1, fh1, fhe1, fh2, fhe2, fhe3, q_eff_lya = rw.read_gas(data_path)
    # initial_grid = pd.read_csv("../input_files/rho_fluc_line0000.txt",skiprows=2,delim_whitespace=True)
    # initial_nh1 = initial_grid["nH1"]
    df_chris = pd.read_csv(data_path,skiprows=2,delim_whitespace=True)
    radius = df_chris[columns[0]]
    #(nh1/fh1) #np.ones_like(temp)*1e-4
    # gas_prop_list= [temp/1e4, (nh1/fh1)*1e4, fh1, np.log10(fhe1), q_eff_lya, np.log10(q_eff_lya*nh1*ne*h*c/lambda_lya_cm/4/pi)]
    ...
    for j in range(rows):
        for k in range(cols):
            gas_prop = df_chris[columns_to_plot[counter]].values
            ...
            ax[j][k].plot(r_relative[mask],gas_prop[mask],color=colors[i],label=str(sim_tot_times[i])+"Myr")
            ...
            ax[j][k].text(0.3,0.5,columns_to_plot[counter],transform=ax[j][k].transAxes)
            counter+=1
            # else:
